# source('./Code/Vulnerability/CoastalDevelopments.R')
...
coastDev <- vulnerability$mob * vulnerability$size

# In matrix
coastDev <- matrix(data = coastDev,
              nrow = nrow(vulnerability), ncol = 1,
              dimnames = list(rownames(vulnerability), 'CoastalDevelopment'))

# Export object as .RData
save(coastDev, file = './Data/StressorVulnerability/CoastalDevelopment.RData')
