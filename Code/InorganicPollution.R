...
# Vulnerability
inorg <- vulnerability$mob * vulnerability$feed

# In matrix
inorg <- matrix(data = inorg,
                nrow = nrow(vulnerability), ncol = 1,
                dimnames = list(rownames(vulnerability), c('InorganicPollution')))

# Export object as .RData
save(inorg, file = './Data/StressorVulnerability/InorganicPollution.RData')
