...
sbt <- vulnerability$env * vulnerability$mob * vulnerability$feed

# In matrix
sbt <- matrix(data = sbt,
              nrow = nrow(vulnerability), ncol = 2,
              dimnames = list(rownames(vulnerability), c('NegativeSBT','PositiveSBT')))

# Export object as .RData
save(sbt, file = './Data/StressorVulnerability/SeaBottomTemperature.RData')
