...
# Vulnerability
nutrient <- vulnerability$mob * vulnerability$feed

# In matrix
nutrient <- matrix(data = nutrient,
                nrow = nrow(vulnerability), ncol = 1,
                dimnames = list(rownames(vulnerability), c('NutrientInput')))

# Export object as .RData
save(nutrient, file = './Data/StressorVulnerability/NutrientInput.RData')
